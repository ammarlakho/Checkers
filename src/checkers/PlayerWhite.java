...
public class PlayerWhite implements MouseListener, KeyListener {
    private final Color c; 
    private Piece movepiece;
    private Piece myinfront, infront, myinfront2, infront2;
    ...
    public PlayerWhite(type type, int depth) {
        super();
        c = Color.WHITE;
    ...
    public void setP2(PlayerRed p2) {
        this.p2 = p2;
    ...
    public void generatePieces() {
        for(int i=0;i<8;i++){
            if(i%2!=0) {
                pieces.add(new Piece(10+50*i,10+50*0, c));
                pieces.add(new Piece(10+50*i,10+50*2, c));
                
           }
            else{
                pieces.add(new Piece(10+50*i,10+50*1, c));
            }
        }
    }
    ...
    public boolean checkMove() {
        ...
        for(int i=0; i<p2.pieces.size(); i++) { //Opponent's piece ahead
            if(p2.pieces.get(i).equals(infront)) {
                flag = false;
            }
        }
        ...
        if(kingrow==1 && movepiece.getY()==360 || kingrow==-1 && movepiece.getY()==10) { //Borders
            flag = false;
        }
    ...
    public boolean checkJump() {
        ...
        for(int i=0; i<p2.pieces.size(); i++) {
            if(p2.pieces.get(i).equals(infront2)) {
                flag2 = true; 
            }
            if((p2.pieces.get(i).equals(infront))) { 
               flag3 = true; 
    ...
    public void makeMove(int col) {
      if(col == 1 || col == -1) {
            ...
            infront = new Piece(movepiece.getX()+50*col, movepiece.getY()+50, Color.RED);
            myinfront2 = new Piece(movepiece.getX()+2*50*col, movepiece.getY()+2*50, c);
            infront2 = new Piece(movepiece.getX()+2*50*col, movepiece.getY()+2*50, Color.RED);
    ...
    public void makeJump(int col){
        for(int i=0; i<pieces.size(); i++) {
            if(pieces.get(i).equals(movepiece)) {
                for(int j=0; j<p2.pieces.size(); j++) {
                    if(p2.pieces.get(j).equals(infront)) {
                        pieces.remove(i);
    ...
    public void moveKing(int col, int kingrow) {
        if((col == 1 || col == -1) && (kingrow == 1 || kingrow == -1)) {
            ...
            infront2 = new Piece(movepiece.getX()+2*50*col, movepiece.getY()+2*50*kingrow, Color.RED);
    ...
    **/
    ...
    public void mouseClicked(MouseEvent e) {
        if(Game.state == Game.STATE.PLAY) {
            int mx = e.getX() - 8;
            int my = e.getY() - 31;
            State state = new State(this, p2);
            ArrayList<Move> moves = state.getAllLegalMoves(State.turn.WHITE);
                ...
                if(turn%2 == 0) {
                    if(this.type == type.human) {
                        ...
                        if(this.type == type.smart) {
                            AI_Move ai_agent = new AI_Move(State.turn.WHITE);
